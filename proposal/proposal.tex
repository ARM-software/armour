...
\paragraph{Applications trust}
Similarly, application trust is obtained form an attestation
procedure linked to software measurements provided by an attestation
service.
%
It is expected that the attestation service for software components
will be carried out by a server running on a trusted execution
environment (TEE).
%
This attestation procedure binds known application instances to
\armour{} logical identities. 

\paragraph{Users}
The notion of a \emph{user} can be application-specific, it can be
linked to a particular infrastructure (Argus, AWS, GC, etc.), it can
be delegated to a third-party authentication service, etc. 
%
Therefore, it is perhaps best to handle user authentication by means
of \armour{} oracles.
%
However, there are some primitive notions of identity that could be
used as a baseline default and perhaps combined with
application/infrastructure specific notions of identity (if any).
%
A-priori \armour{} does not need to have  primitive notion of user
identity, but this aspect need to be to be studied further.

\bigskip

While these notions of identity are related to a physical or
measurable entities, or delegated to specific oracles, there will be
other notions of identity which will be fundamentally logical.
%
...
\end{inparaitem}
%
\gpn[How? Labels? Identity Types?]{To accommodate for these entities
  we consider that \armour{} should be able to treat user-defined
  identity notions in the policies.}
...
%
On the other hand, the design of \armour{} requires that all
(communicating) logical notions of identity be backed PKI certificates
...
high-level identity primitives manipulated by \armour{}, and it
validates the security achieved by mTLS in the data-plane. 
...
\begin{table}[t]
  ...
  \begin{tabular}[t]{l|l}
    ...
    Defensive programming (Sanitization)     & Service types                      \\
    Lightweight Types                        & User-defined Roles                     \\
    Service compartmentalization             &                \\
    Control/Data plane separation            &                                   \\
    Native RoT verification                  &                                   \\
    mTLS throughout                          &                                   \\
    Crypto-backed services (enclaves?) & 
  \end{tabular}
...
Common security related software errors from~\cite{Wing99}:
\begin{itemize}
\item race conditions on resource accesses, 
\item not checking data content and size, 
\item not checking success/failure results, 
\item omission of resource exhaustion checks,
\item not checking operating environment assumptions, 
\item incorrect use of system calls, 
\item incorrect use of system modules.
\end{itemize}
...
\newcommand{\dpState}{\ensuremath{\Omega}}
\newcommand{\adpState}{\ensuremath{\omega}}
\newcommand{\RuleSet}{\ensuremath{\mathcal{R}ules}}
\newcommand{\adpid}{\ensuremath{p_{i\!d}}}
\newcommand{\DPId}{\ensuremath{D\!P_{I\!d}}}
\newcommand{\apol}{\ensuremath{p}}
\newcommand{\policies}{\ensuremath{\mathcal{P}ol}}
\newcommand{\compiler}{\ensuremath{\mathcal{C}}}
\newcommand{\oracleState}{\ensuremath{\mathcal{O}\!_{st}}}
\newcommand{\gdpState}{\ensuremath{D\!P\!_{st}}}
\newcommand{\Request}{\mathcal{R}eq}
