...
use super::expressions::{Block, CPExpr, DPExpr, Error, Expr, self, Pattern};
use super::externals::{Call, ExternalActor};
use super::headers::{self, CPHeaders, Headers, THeaders};
use super::interpret::{CPEnv, TInterpret};
...
impl<FlatTyp:TFlatTyp, FlatLiteral:TFlatLiteral<FlatTyp>> Expr<FlatTyp, FlatLiteral> {
    //TODO is env usefull
    fn simplify(self, env: Env<FlatTyp, FlatLiteral>) -> Result<Self, self::Error> {
        match self {
            Expr::Var(_) | Expr::BVar(_, _) => Ok(self),
            Expr::LitExpr(_) => Ok(self),
            Expr::Closure(x, e) => Ok(Expr::Closure(x, e.simplify(env)?)),

            Expr::ReturnExpr(e) => Ok(Expr::ReturnExpr(e.simplify(env)?)),
            Expr::PrefixExpr(p, e) => match e.simplify(env)? {
                r @ Expr::ReturnExpr(_) => Ok(r),
                Expr::LitExpr(l) => match l.clone().eval_prefix(&p) { //FIXME move eval prefix as simplify prefix
                    Some(lit) => Ok(lit),
                    None => Ok(Expr::PrefixExpr(p, e)),
                },
                ee => Expr::PrefixExpr(p, ee),
            },
            Expr::InfixExpr(op, e1, e2) => {
                let r1 = e1.simplify(env.clone());
                let r2 = e2.simplify(env);

                match (r1, r2) {
                    (r @ Expr::ReturnExpr(_), _) => r,
                    (_, r @ Expr::ReturnExpr(_)) => r,
                    //Boolean
                    (Expr::LitExpr(l), e) | (e, Expr::LitExpr(l)) if p==Infix::And && l.is_bool() => {
                        if l.get_bool() { 
                            Ok(e.simplify()?)
                        } else {
                            Ok(Expr::LitExpr(l))
                    ...
                    //Boolean
                    (Expr::LitExpr(l), e) | (e, Expr::LitExpr(l)) if p==Infix::Or && l.is_bool() => {
                        if l.get_bool() { 
                            Ok(Expr::LitExpr(l))
                        } else {
                            Ok(e.simplification()?) 
                    ...
                    (Expr::LitExpr(l1), Expr::LitExpr(l2)) => match l1.eval_infix(&op, &l2) {
                        Some(r) => Ok(r.into()),
                        None => Err(Error::new("simplify infix: type error")),
                    },
                    _ => Ok(Expr::InfixExpr(op, e1, e2)),
                }
            }
            Expr::BlockExpr(b, es) => {
                if es.is_empty() {
                    ...
                    let res = e.simplify(env.clone()).await?;
                    if res.is_return() || es.is_empty() {
                        Ok(res)
                    } else {
                        Expr::BlockExpr(b, es).simplify(state, env).await
                    }
                } else {
                    // list or tuple
                    let mut rs = Vec::new();
