...
tcp_endpoints=(172.34.0.2:4713 172.27.0.2:1883 172.26.0.2:1883 172.25.0.2:1883 172.22.0.2:4713)
touch proxy_map
echo "launch" > proxy_map
echo "wait 1" >> proxy_map
echo "start "$rest_port >> proxy_map
echo "wait 1" >> proxy_map
...
for i in "${tcp_endpoints[@]}"; do
  ...
  echo "forward "$tcp_port" "$i >> proxy_map
  echo "wait 1" >> proxy_map
  let "tcp_port++"
done
echo "allow all" >> proxy_map
