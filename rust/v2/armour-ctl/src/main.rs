...
async fn main() -> std::io::Result<()> {
    ...
    let cp_url = matches.value_of("CONTROLPLANEURL")
    ...
    if let Some(update_matches) = matches.subcommand_matches("update") {
        let file = update_matches.value_of("POLICYFILE")
            .ok_or(Error::new(ErrorKind::Other, "Wrong policy file"))?;
        let service = update_matches.value_of("SERVICE")
            .ok_or(Error::new(ErrorKind::Other, "Wrong service"))?;
    ...
    if let Some(query_matches) = matches.subcommand_matches("query") {
        let service = query_matches.value_of("SERVICE")
            .ok_or(Error::new(ErrorKind::Other, "Wrong service"))?;
        ...
        let body = req.body().await
            .map_err(|_| Error::new(ErrorKind::Other, "Server error"))?;
        match json::parse(std::str::from_utf8(&body)
                          .map_err(|_|
                                   Error::new(ErrorKind::Other, "Parse policy error"))?)
            .map_err(|_|
                     Error::new(ErrorKind::Other, "Parse policy error"))?
           {
            JsonValue::String(s) => {
    ...
    if let Some(master_matches) = matches.subcommand_matches("fake-master-onboard") {
        let master_url = master_matches.value_of("MASTERURL")
            .ok_or(Error::new(ErrorKind::Other, "Wrong master URL"))?;
        let masterlabel = master_matches.value_of("MASTER")
            .ok_or(Error::new(ErrorKind::Other, "Wrong master label"))?;
    ...
    if let Some(service_matches) = matches.subcommand_matches("fake-service-onboard") {
        let service = service_matches.value_of("SERVICE")
            .ok_or(Error::new(ErrorKind::Other, "Wrong service"))?;
        let master = service_matches.value_of("MASTER")
            .ok_or(Error::new(ErrorKind::Other, "Wrong master"))?;
