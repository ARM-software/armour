...
pub fn proxy(
    ...
    policy.send(policy::Check::Require).then(|b| {
        match b {
            Ok(true) => future::Either::A(
                policy
                    ...
                    .then(move |res| match res {
                        // allow request
                        Ok(Ok(true)) => {
                            future::Either::A(request(req, payload, policy, client, proxy_port))
                        }
                        // reject
                        Ok(Ok(false)) => {
            ...
            // reject request in absense of check
            Ok(false) => future::Either::B(future::Either::A(future::ok(unauthorized(
                "request denied",
            )))),
            // actor error from sending message Check::Require
            Err(err) => {
                warn!("{}", err);
                future::Either::B(future::Either::B(future::ok(internal())))
            }
        }
    })
}
...
pub fn request(
    ...
    policy.send(policy::Check::ClientPayload).then(|b| {
        match b {
            ...
            Ok(false) => future::Either::B(future::Either::A(
            ...
            // actor error from sending message Check::ClientPayload
            Err(err) => {
                warn!("{}", err);
                future::Either::B(future::Either::B(future::ok(internal())))
            }
...
pub fn response(
    ...
    match res {
        Ok(mut res) => {
            ...
            future::Either::A(policy.send(policy::Check::ServerPayload).then(|b| {
                match b {
                    Ok(true) => future::Either::A(
