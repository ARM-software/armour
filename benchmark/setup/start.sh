#!/bin/bash
cd ~/Downloads
aws ec2 run-instances --image-id ami-07ed3aa77a800b3fe --count 63 --instance-type t2.large --key-name some-key --security-groups arm-default --region eu-west-2
sleep 120s
aws ec2 describe-instances --filters "Name=image-id,Values=ami-07ed3aa77a800b3fe"  --region eu-west-2 --query 'Reservations[*].Instances[*].NetworkInterfaces[*].PrivateIpAddresses[*].[Association.PublicIp]' --output text > ips
...
cd /home/ec2-user/scripts
screen -d -m -S test ./test.sh armour latency all-async-log
SHELL
elif [ "$i" -le 18 ]; then
  ssh -i ~/Downloads/some-key.pem -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no ec2-user@$ip <<SHELL
cd /home/ec2-user/binaries
...
screen -d -m -S log ./logger log_sock
cd /home/ec2-user/scripts
screen -d -m -S test ./test.sh armour latency async-log
SHELL
elif [ "$i" -le 27 ]; then
  ssh -i ~/Downloads/some-key.pem -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no ec2-user@$ip <<SHELL
cd /home/ec2-user/binaries
