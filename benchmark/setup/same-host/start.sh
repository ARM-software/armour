#!/bin/bash
cd ~/Downloads
aws ec2 run-instances --image-id ami-0bb340616b7c666a0 --count 57 --instance-type t2.large --key-name some-key --security-groups arm-default --region eu-west-2
sleep 120s
aws ec2 describe-instances --filters "Name=image-id,Values=ami-0bb340616b7c666a0"  --region eu-west-2 --query 'Reservations[*].Instances[*].NetworkInterfaces[*].PrivateIpAddresses[*].[Association.PublicIp]' --output text > ips
